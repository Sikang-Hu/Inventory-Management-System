...
(
    ...
    region_id  INT,
    wh_name    VARCHAR(30),
    wh_address VARCHAR(255),
    wh_state   CHAR(2),
    wh_zip     INT,

    CONSTRAINT wh_fk_region FOREIGN KEY (region_id) REFERENCES region (region_id)
);
...
(
    dist_id       INT AUTO_INCREMENT UNIQUE,
    wh_id         INT,
    store_id      INT,
    shipment_date DATETIME NOT NULL,
    arrival_date  DATETIME,

    PRIMARY KEY (dist_id, wh_id, store_id),
    CONSTRAINT dist_fk_wh FOREIGN KEY (wh_id) REFERENCES warehouse (wh_id),
    CONSTRAINT dist_fk_store FOREIGN KEY (store_id) REFERENCES retail_store (store_id)
);


DROP TABLE IF EXISTS distribution_has_item;
CREATE TABLE IF NOT EXISTS distribution_has_item
(
    dist_id           INT,
    item_id           INT,
    ...
    PRIMARY KEY (dist_id, item_id),
    CONSTRAINT di_fk_dist FOREIGN KEY (dist_id) REFERENCES distribution (dist_id),
    CONSTRAINT di_fk_item FOREIGN KEY (item_id) REFERENCES item (item_id)
);
*/