...
//            // TODO: SOLUTION: just insert relationship with existing items and return a list
//            //  of new items
//            con.commit();
//        } catch (SQLException e) {
...
//    }
    ...
    List<ItemDTO> addSoldItem(String vendorName, Iterable<ItemDTO> items) {
        Statement  stmt = null;
        Connection con = null;
        List<ItemDTO> itemsHaveNoRecords = new ArrayList<>();
        try {
            con = DatabaseUtil.createConnection();
            stmt = con.createStatement();
            int ven_id = new VendorDAO().getVendorByName(vendorName).getVendorID();
            con.setAutoCommit(false);
            for (ItemDTO i : items) {
                try {
                    int item_id = new ItemDAO().getItemByName(i.getItemName()).getItemId();
                    String sql = "INSERT INTO vendor_has_item (ven_id, item_id) VALUES (" + ven_id + ", "
                            + item_id + ")";
                    stmt.executeUpdate(sql);
                } catch (IMSException e) {
                    itemsHaveNoRecords.add(i);
                }
            }
            con.commit();
            return itemsHaveNoRecords;
        } catch (SQLException e) {
            DatabaseUtil.rollback(con);
            e.printStackTrace();
            throw new IMSException(e.getMessage());
